...
class UMNNMAF(nn.Module):
    ...
    # Kind of dichotomy with a factor 100.	    def invert(self, z, context=None):
    def invert(self, z, iter=10, context=None):
        nb_step = 10
        step = 1 / (nb_step - 1)

        x_range = (torch.ones(z.shape[0], nb_step) * torch.arange(0, 1 + step / 2, step)).permute(1, 0).to(
            self.device)
        ...
        with torch.no_grad():
            for j in range(self.input_size):
                ...
                h, offset = h.squeeze(1).unsqueeze(0).expand(nb_step, -1, -1), offset.unsqueeze(0).expand(nb_step,
                                                                                                          -1, -1)
                ...
                for i in range(iter):
                    ...
                    z_est = s[:, :, [j]] * (offset + ParallelNeuralIntegral.apply(x0, x[:, :, j].contiguous().view(-1, 1),
                                                                                  derivative, None, h.contiguous().view(x0.shape[0], -1), self.nb_steps).contiguous().view(nb_step, -1, 1))
